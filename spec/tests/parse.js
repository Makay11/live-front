...
describe("the 'parse' function", function () {
  ...
  it("should correctly parse the first test file's contents (.jade)", function (done) {
    lf.parse(fs.readFileSync(testFiles[0])).then(function (result) {
  ...
  it("should correctly parse the second test file's contents (.html)", function (done) {
    lf.parse(fs.readFileSync(testFiles[1])).then(function (result) {
      expect(result.object).toEqual({
        a: {
          b: "hi",
          c: [
  ...
  it("should correctly parse the third test file (.jade)", function (done) {
    lf.parse(fs.readFileSync(testFiles[2])).then(function (result) {
